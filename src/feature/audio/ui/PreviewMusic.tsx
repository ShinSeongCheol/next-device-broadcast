...
import {WaveForm} from "@/src/feature/audio";
...
export default function PreviewMusic({audioDetails}: Props) {
    ...
    return (
        <div className={'flex flex-col gap-4'}>
                    ...
                    {audioDetails.map((audioDetail) => {
                        return (
                            <div key={audioDetail.id} className={'flex flex-col gap-2 w-full max-w-48 min-w-0'}>
                                <AspectRatio ratio={1} className="relative w-full rounded-lg bg-neutral-800 group flex items-center justify-center" onClick={() => handleClick(audioDetail.uuid)}>
            ...
            {uuid && <WaveForm key={uuid} uuid={uuid}/>}
        </div>
    )
}